...
class Constants {
    companion object {
        ...
        private const val PRIVATE_KEY = "1b82f41e168d650fdc913a82328c1dc12cd230af"
        const val limit = 100

        private const val radix = 16
        private const val padStart = 32

        fun hash(): String {
            val input = "$ts$PRIVATE_KEY$API_KEY"
            val md = MessageDigest.getInstance("MD5")
            return BigInteger(1, md.digest(input.toByteArray())).toString(radix)
                .padStart(padStart, '0')
        }
